...
public class TestTableCleaner {
     ...
     * 2026-09-17 我在这里犯过一次错：写成对常量判前缀后，构造器的 {@code expectedDatabase}
     * 成了**死参数**，而 {@code INFRA_table_cleaner_refuses_non_test_database} 那条
     * "故意声明一个不存在的期望库名 → 必须拒绝"的断言**再也不可能抛异常** ——
     * 一条还能失败的有效断言被我改成了永远通过的断言。是 W3-M3b 任务复核时定位出来的。</p>
     *
     * <p>⚠️ <b>另一个教训（同一天，代价更大）</b>：改这个文件时我曾用
     * {@code Get-Content -Raw} + {@code Set-Content -NoNewline} 做变异验证 ——
     * PowerShell 5.1 的 {@code Get-Content} 按 GBK 读 UTF-8，写回去就是**二次编码**，
     * 结果 36 行中文注释变成非法 UTF-8，`mvn test` 在 test-compile 阶段就失败，
     * **整个项目所有人的测试都跑不起来**，而我在提交时只核了行数没核编码。
     * **源码文件一律只用编辑工具改；变异验证要用 cp 备份 + cp 还原。**</p>
     *
     * @param expectedPrefix 本次实例期望的库名前缀（构造器传入）
