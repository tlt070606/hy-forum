...
| 端口范围 | **18091/18091** |
| 授权对象（源） | `0.0.0.0/0` |
| 描述 | `Hy论坛 OSS 回调（开发期，仅放行 POST /api/oss/callback）` |

**关于 `0.0.0.0/0` 的取舍（如实说明）**：源地址开放到全网，**但公网能看到的东西只有一个路径**，
而它自身有 OSS 签名校验（技术方案 §8.4）；其余路径 444。**这比"把开发机后端整站暴露"安全得多**，
但仍属开发期临时设施 —— **M3 验收完就应停掉**（或至少在不再需要远程回调时停掉）。

> 若想收得更紧：阿里云 OSS 的回调来源没有公开的稳定小网段，按源 IP 白名单不可行。
> **真正收口的办法是回调不依赖公网**（例如上线后由正式域名的 HTTPS 入口承担），那属于 M8。
...
| 3 | **回调公网地址**：`http://8.138.237.212:18091/api/oss/callback` | ✅ **已搭好**（§5），但**安全组还没放行 → 目前从公网不可达** |
| — | **Bucket 的 CORS 是否已配** | ❌ **未验证**（§2） |
| — | **RAM 子账号与最小授权策略是否已建** | ❌ **未验证**（§3） |
| — | **AccessKey 是否已放进环境变量／忽略文件** | ❌ **未验证**（§4，且**不要发给我**） |
| — | **安全组放行 18091/TCP** | ⛔ **只剩这一步**，只有需求方能做（§5.4） |
...
| **公网可达** | 2026-09-15 | ⛔ **未通过** —— `POST http://8.138.237.212:18091/...` → HTTP 000；对照 `GET http://8.138.237.212/` → 200（证明差别只在安全组规则） | §5.4 |
| 安全组放行 18091/TCP | | ⬜ **待需求方执行** | §5.4 |
| §7 第 1 步 CORS（浏览器验，不是 curl） | | ⬜ 未做 | §2 |
| §3 策略已授予 RAM 用户 | | ⬜ 未做 | §3 |
| Bucket 名 / 地域 / 回调地址 | 2026-09-15 | ✅ `hy-forum-2026` / 华北2（北京）`oss-cn-beijing.aliyuncs.com` / 见 §6 | §6 |
