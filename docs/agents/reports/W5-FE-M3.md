...
# W. 后端进程"消失"事件（2026-09-19 20:21，虚惊一场但要留档）

## W.1 现象与真相

后台作业 `pwsh-178`（我起的 8080 实例）报 **completed / exit 1**，
日志末尾停在 `2026-09-19T20:20:07` 一条**正常的用户注册**，**没有任何 Java 异常**。

但核实后发现：**8080 一直有服务**，只是**换了进程** ——
监听者是 **PID 15324，启动于 `09-19 20:21:29`**（不是我那个 PID 29016），
而且它的 `callbackUrl` 仍是公网地址 `http://8.138.237.212/api/oss/callback` ✓
→ **有人（需求方或 L1）在我检查前 20 秒起了新实例，配置是对的。**
所以**我没有再去起一个**（抢 8080 只会更糟）。

## W.2 两条值得记的环境事实

1. **"后台作业结束" ≠ "它启动的服务结束"**。这次是**作业外壳**先没了，java 子进程还活着
   （也能反过来：外壳活着、子进程被别的力量杀掉 —— 见 §N.5）。
   所以判断服务状态**永远要查端口 + 打一次接口**，不要看作业状态、
   也**不要只看启动日志**（§N.5 已因这个误判过一次）。
2. **本机会休眠**。旧实例日志里有一串 `HikariPool - Thread starvation or clock leap`
   （`delta=4h36m`、`14h52m`、`42m`）与随之而来的 Redis 断连/重连
   （`192.168.100.128:6380` 先 `Connection reset`、后 `No route to host`，最终自动重连成功）。
   **长时间空闲后这些是正常现象**，看到 `No route to host` 不必当成故障。

## W.3 顺手复核的两件事（都正常）

- **信息流干净**：可见帖 16 条，其中**我的测试帖 0 条**。
  而且 09-19 20:19/20:20 那两次运行（有人跑了 `m3-upload` 用例）**在库里没留下任何帖子** ——
  **`ac23376` 给用例加的自清理确实生效了**（以前那样跑一次就会多一条"带图帖"）。
- **服务全绿**：后端 8080 = 200、前端 5173 = 200、MySQL 可查、Redis 6380 可连。

## W.4 遗留的清理债（不影响信息流，但该有人管）

`user` 表里累积了 **360 个 `e2e_*` 测试账号**（每跑一次用例注册几个）。
**界面上看不到它们**（不进信息流、不进推荐关注 —— 后者是 mock），但属于测试痕迹。
**建议**：清掉"**没有任何内容**（帖子/评论/收藏全无）的 e2e 账号"；
有内容的（例如 L1 那两条"读时签名证据帖"的作者）**要单独判断**，不能一律删。

---

# X. M5 前端（通知 + 举报）

## X.0 ⚠️ 先说范围是怎么定的：**M5 没有前端任务书**

需求方让我"开始 M5 的前端"，但按开工规矩查了三处：
- `docs/agents/tasks/W5-M5-通知与审核出口.md` —— **派给的是"后端 L2 实现者"**，
  写权在 `server/src/main/java/com/hyforum/{notify,audit,admin}/**`，
  而且明文写着「**禁止改 `openapi.json`／`schema.sql`／`web/**`；禁止读 `web/**`**」；
- `docs/PLAN.md` 的 M6 才定义"管理后台 = **独立前端工程**"；
- 契约已经前移到 **35 路径 / 63 schema**，含 M5 的全部端点。

**所以我按"契约能支撑什么 + PLAN 的 M5 验收里哪半是用户可见的"来定范围**
（这是看板那两条协作规矩里的"写成我采用的读法+理由继续做"，不是停下等人）：

| 做 | 不做（说明） |
|---|---|
| **消息通知**：列表（分页/按类型筛选）、未读数、单条与全部已读 | `/api/admin/comments*` 等后台端点 → **M6 的独立管理后台工程**，不在这个端里 |
| **举报**：帖子 + 评论（弹层选原因 + 补充说明） | 内容安全（敏感词库）→ 已被需求方裁定移出本版 |
| 未读红点接到**真接口**（此前是写死的假数据 3） | |

**⚠️ "M5 没有前端任务书"这件事本身要 L1 确认**：我按上面这份读法做完了，
如果与你的意图不符，改动是可回退的（都在 `web/**`）。

## X.1 做了什么

| 文件 | 内容 |
|---|---|
| `api/notifications.ts`（新） | 列表 / 未读数 / 标记已读 |
| `api/report.ts`（新） | 举报（含 `REPORT_TARGET` / `REPORT_REASONS` 两组**逐条对应契约取值**的常量） |
| `api/shape.ts` | 新增 `expectNumber` —— 见 X.3 第 1 条 |
| `api/types.ts` / `api/contract.ts` | 登记 M5 的 4 个端点与具名类型 |
| `stores/notify.ts`（新） | **顶栏与消息页共享的未读数**（两处各请求一次会出现"顶栏 3、页面 2"） |
| `utils/notifyView.ts`（新） | 通知归一化：类型→图标/颜色、**兜底文案**、跳转目标 |
| `pages/notifications/index.vue`（新） | 消息页：类型筛选、列表、未读圆点、单条已读、全部已读、分页 |
| `components/ReportSheet.vue`（新） | 举报弹层（帖子与评论共用） |
| `pages/post/detail.vue` | 举报按钮接真接口（原来只提示"将在 M5 交付"） |
| `components/CommentSection.vue` | 每条评论可举报（`targetType=2`） |
| `components/shell/TopBar.vue` | 铃铛接**真未读数**（删掉 `MOCK_UNREAD_NOTIFICATION_COUNT`） |
| `components/shell/MobileNav.vue` | **新增「消息」入口 + 红点** —— 见 X.2 |

## X.2 🔴 E2E 抓到的真问题：**手机上根本进不去消息页**

窄屏下顶栏的铃铛被 `display:none` 隐藏（原注释写着"底部导航才是移动端主入口"），
而底部导航里**原本只有 首页/话题/收藏/我的 —— 没有消息**。
也就是说：**做完了消息功能，手机端却没有任何入口能到达它。**

修法：底部导航加「消息」（5 个入口）+ 未读红点（与顶栏**共用同一个 store**，
所以两处数字不会打架）。E2E 里两条断言分别钉住"入口存在"与"红点同源"。

> 原注释的假设（"底部导航是移动端主入口"）对通知**不成立** ——
> 这是"注释里写着的前提，功能一变就失效"的典型，只有真跑起来才会暴露。

## X.3 两个实现上的判断（都值得记）

1. **未读数与标记已读的 `data` 是"一个裸数字"**（`ApiResponseLong`），不是对象。
   用 `expectObject` 收敛会直接抛"形状不对"。所以加了 `expectNumber`，并明确：
   `null`/`undefined` → 视为**无法识别**（**不能当 0**：0 是"确实没有未读"，与"没拿到"语义相反）。
2. **`NotificationVO.content` 实测是 `null`**（探针打真后端：关注类通知的 `content` 就是 null）。
   所以前端**必须有兜底文案**（`档k12w 关注了你`）—— 我无法从契约文字判断它是否总会有值，
   于是按"可能为空"处理。**E2E 断言的正是那句兜底文案**，不是后端文案。

## X.4 🔴 CR-N：评论类通知**点不进去**

契约 `NotificationVO` 只给 `targetType`(1帖子/2评论) + `targetId`：
- `targetType=1` → `targetId` 是帖子 id → 能跳详情 ✓
- `targetType=2` → `targetId` 是**评论 id**，而**没有 postId、也没有"评论 id → 帖子"的接口**
  → 前端**无法定位到那条评论所在的帖子**。

**处置**：这类通知**不做假跳转**（跳到一个猜出来的地址比不跳更糟），
点击只标记已读并**明确告知原因**。
**建议**：`NotificationVO` 补 `postId`（评论类通知带上它即可，一个字段解决）。

## X.5 已知残留（如实记）

- **每次跑举报用例会在 `report` 表留一行**：契约里没有"撤销/删除举报"的接口，
  所以 E2E **清不掉**它（关注关系可以用 `DELETE /api/follow/{id}` 解除 ✓，那部分清了）。
  界面上看不见它（后台举报列表属 M6），但它是真实残留。
- 探针若用了不存在的帖子 id 会返回 404（我第一次探针就踩了），**这不是缺陷**，是探针写错了 id。

## X.6 验证

```
npx vue-tsc --noEmit -p tsconfig.json   → exit 0
npx playwright test                     → 29 passed (56.0s)
    原 25 条 + 新增 M5 4 条（未登录不发请求 / 红点与已读 / 全部已读 / 举报）
```

**一条测试侧的坑（值得全项目抄走）**：断言"某接口有没有被调用"时，
**不能用 `url().includes('/api/notifications')`** —— H5 的模块路径是
`/src/api/notifications.ts`，它**包含**这个子串，于是"模块加载"被数成了"API 调用"，
**一条真请求都没发也会红**。必须锚在 pathname 上（`new URL(url).pathname.startsWith(...)`）。
本机为此白绕了一圈（探针里才看清 `token = null`、根本没有真请求）。

---

# Y. 假数据清理：左栏「话题」→「版块」；右栏三块假数据全换真数据（2026-09-18 需求方裁定）

## Y.1 需求方原话与 grill 结果

> 「前端的那个话题可以不要，你有什么建议，换成有的后端接口」

| # | 问题 | 定论 |
|---|---|---|
1 | 左栏「话题」换成什么 | **「版块」**（`GET /api/boards` 真接口；`BoardVO` 有 `name/description/postCount/isResource`，够用） |
2 | 右栏三块假数据怎么办 | **全换**：版块榜（`/api/boards` 按 `postCount` 排）+ 最新发帖的人（`/api/posts` 作者去重）；**热门活动删掉**（没有接口） |
3 | 死页面怎么处理 | **删掉页面文件与路由** |
4 | 发帖页要不要「话题」 | **不加**（帖子表里没有话题字段，加就是改契约） |

## Y.2 做了什么

| 文件 | 改动 |
|---|---|
| `pages/boards/index.vue`（新） | **版块总览**：全部版块 + 简介 + 真实帖子数 + 资源版块标记；顺序**就是接口给的顺序**（`sort` 由服务端定，前端不重排） |
| `LeftRail.vue` | 话题→版块；**删掉假数据「活跃用户 8」**（契约里没有任何行为/统计接口能做真，假的一律去掉） |
| `MobileNav.vue` | 话题→版块（**必须一起改**：`/pages/topic` 的路由已删，留着就是**死链**） |
| `RightRail.vue` | 重写：**版块榜**（真 `postCount`）+ **最新发帖的人**（真作者，去重取前 5）；删掉热门话题榜/推荐关注/热门活动三块 |
| `pages.json` | 删 `pages/topic/index` 路由、加 `pages/boards/index` |
| `pages/topic/index.vue` | **删除**（占位页，无表无接口） |
| **`src/mock/` 整个目录** | **删除** —— 前端**不再有任何编造的数据** |

**顺带说清一处刻意的"不做"**：右栏「最新发帖的人」**没有放关注按钮**（原来的假数据块是有的）——
`isFollowing` 只有 `UserProfileVO` 带（`/api/users/{id}`），列表项作者是 `UserBriefVO`，
**没有这个字段**；放了就是"点完刷新又变回未关注"的假按钮（与 CR-K 同源）。
要看某人、要关注他，点进主页做，那里的状态是真的。

## Y.3 ⚠️ 我在这轮犯的错（值得记进环境陷阱）

**我用"正则批量删除"去删模板块，把 `LeftRail.vue` 的模板删坏了** ——
删完留下 `<view    <view v-if="hasPostTotal" class="divider" />` 这种拼接残骸，
Vite 编译该模块直接 **HTTP 500** → **整个首页渲染失败**（不只是左栏）。

**更值得记的是我排查的顺序错了**：
1. 先跑 E2E 看到"元素找不到"，我**猜**是"窄屏不渲染左右栏"（改了视口，还是不对）；
2. 又猜是 `postCount` 可选类型之类的小问题（改了，还是不对）；
3. 最后**用探针打控制台**，一眼看到 `[http 500] /src/components/shell/LeftRail.vue`
   与 `TypeError: Failed to fetch dynamically imported module` —— 根因在这里。

**两条教训**：
- **删模板代码必须用精确字符串替换，不要用正则**（模板不是行结构，正则会把标签切碎）；
- **"页面上大量元素都找不到"时，第一件事是看控制台有没有 500/编译错误**，
  而不是先怀疑视口、类型、CSS —— 那三次猜测浪费的时间比直接看日志多得多。

## Y.4 验证

```
npx vue-tsc --noEmit -p tsconfig.json   → exit 0
npx playwright test                     → 31 passed (56.4s)
    原 29 条 + 新增 2 条（左栏换项与版块总览数量 / 右栏真数据与旧假数据 testid 必须为空）
```

新增用例的断言口径也换了：**不再断言"界面上有个数字"，而是断言"界面上那个数 == 接口给的那个数"** ——
只断言"有内容"是抓不到"又变回假数据"的。

---

# Z. 头像与个人简介：**契约里没有写接口，做不了**（2026-09-19）
...
```
仓库 openapi.json：paths = 35
/api/user/** 只有 GET：/api/user/me、/api/user/collections
**没有任何 PUT / PATCH 到 /api/user/****
唯一能写的用户接口是 POST/DELETE /api/follow/{userId}
```

所以"改头像 / 改简介"**不是我不做，是没有接口可打**。需求方 grill 后确认：
**等 L1 加接口**，要的字段是 **`avatarUrl` + `bio` 两个**（不动 nickname、不加审核状态）。

## Z.2 🔴 CR-O（需要契约补充）：`PUT /api/user/profile`

**给 L1 的一句话**：

> 请补 `PUT /api/user/profile`（需登录，只改自己的）：
> 请求体 `{ avatarUrl?: string, bio?: string }`（**只这两个字段**即可），
> 响应回 `UserVO`（与 `GET /api/user/me` 同形）。`avatarUrl` 的值来自 OSS 直传回调返回的
> `url`（与帖子图片同一套，前端已有完整实现）；`bio` 纯文本，建议 ≤ 200 字。
> 昵称本期不改（它是评论/通知里到处引用的显示名，改它要考虑旧数据）。

**前端这半已经就绪**（所以接口一到，接线是十几分钟的量级）：
- **上传**：`/api/oss/signature` + 浏览器直传 + 回调落库 —— 就是发帖带图用的那套，已在 §6.1 闭环里验证过；
- **回显**：`GET /api/user/me` 已返回 `avatarUrl` / `bio`，头像组件（`Avatar.vue`）三种形态都做好了；
- 缺的只是"把新值写回去"那一个端点。

## Z.3 顺带修掉一处**过期的文案**（这个必须现在改）

「我的」页与设置页原先把"改头像/改简介/改密码"标注为「**待 M5 交付**」——
**但 M5 已经交付完了，而这三个接口仍然不存在** → 那句话现在是**错的**，
会让需求方以为"再等等 M5 就有了"。已改成准的：

- 头像：「**头像编辑待后端接口**」
- 简介：「编辑简介与头像需要后端接口，**契约里目前没有**（已提 CR：PUT /api/user/profile，字段 avatarUrl + bio）→ 接口到位即可用」
- 密码：「修改密码需要后端接口，**契约里目前没有** → 待交付」

对应 E2E 断言同步更新（`m4-settings.spec.ts`，3 passed）。

> 一条通用教训：**提示语里的里程碑编号会过期**。
> "待 M5 交付"在 M5 交付当天就变成假话 —— 写"待后端接口"这种**不依赖时间**的说法更耐用。

## Z.4 本轮另两件事

1. **提交信息带 BOM（我的疏忽，不 amend）**：`659aa4e` 的主题行前面有一个不可见的 BOM ——
   我用 `Set-Content -Encoding UTF8` 写提交信息文件，**PS 5.1 建新文件时会加 BOM**。
   这条坑项目里早就记过（要用 write 工具写 UTF-8 no BOM），我又踩了一次。
   按规矩**不 amend**，在此披露（与 §11 的 `a53074a` 同类）。
2. **一条 flake 如实记**：全量跑时 `m3-upload`（§6.1 最小闭环）出现**一次**瞬时失败，
...
# EE. 收藏卡片补上作者；列表九宫格落地（L1 已补 `author` / `imageThumbs`）

需求方 2026-09-21：「好，接着做」（接的是我上一轮问的"收藏卡片加作者头像"）。

## EE.1 ✅ 收藏卡片现在与首页卡片**结构完全一致**

`CollectionItemVO` 补上 `author`（`UserBriefVO`）之后：

- 收藏卡片**显示作者行**（头像 + 昵称）—— 这正是需求方要的"有头像"；
- **「收藏于 X」放在作者行的位置**（需求方：「收藏的时间就放在头像旁边」），
  即首页卡片里"发帖时间"的那一格 → 两张卡片连结构都对齐了；
- 旧断言（"收藏卡片**不该**有作者行"）已**反过来**：现在断言**必须有且非空**。

## EE.2 ✅ 列表九宫格（需求方 2026-09-18 的老要求，今天终于能做）

`PostSummaryVO`/`FeedItemVO` 有了 `imageThumbs`（前最多 3 张，读时签名）→ 列表可以画真九宫格：

| 情况 | 渲染 |
|---|---|
**≥2 张签名缩略图** | 九宫格（最多 **3** 格），超过 3 张的部分用「**+N**」角标（N 由 `imageCount` 算，**不编地址**） |
**只有 1 张 / 拿不到缩略图** | 退回原有的一格 + 「N 张」角标 |

⚠️ **为什么不画满 9 格**：契约明确只返回**前 3 张** —— 硬画 9 格就得自己造地址（违反口径 7）。
新增 `e2e/m5-card-grid.spec.ts`：按接口挑一篇真有 ≥2 张缩略图的帖，
断言卡片画了九宫格、格子数 = `min(3, 缩略图数)`、**每个格子都真的加载出来**（查网络响应）、
且**没有**同时渲染单格封面那一套（防止两条分支都出现）。

## EE.3 🔴 CR-S：**收藏项的缩略图 URL 参数顺序不对 → 403**

同一个对象，两个接口给的缩略图地址**参数顺序不同**，后者读不了：
...
（同一帖的 coverUrl 两边都是 200 ✓）
```

**所以收藏页我刻意不给缩略图**（`imageThumbs: []`，退回 `coverUrl` 单格）——
与其画一堆 403 的灰块，不如老实显示那一格。**要 L1 把收藏项的缩略图按列表那样拼**（签名覆盖 `x-oss-process`）。

## EE.4 一个我踩了两次的坑（记下来）

**判断"图有没有加载出来"不能读 `naturalWidth`**：uni-app H5 的 `<image>` 渲染成
`<uni-image>` 外壳 + 内部一个用 `background-image` 的 **div**，**根本没有 `<img>` 子元素** ——
`querySelector('img')` 永远是 null。改用**网络响应**判断（与实现无关，且验的正是"拉下来了且没失败"）。
（第一次是查头像时踩的，这次是九宫格。）
