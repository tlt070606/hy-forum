 ...
 *
 * ==========================================================================
 * 为什么需要它（而不是用 `openExternalLink()` 的返回值去"探测"）
 * ==========================================================================
 * `openExternalLink()` 在 H5 端会**真的弹出一个窗口**，所以它是一个**动作**，
 * 不是能力探测函数 —— 拿它来预判"能不能打开"，会在渲染期就把浏览器窗口弹出来。
 * 因此把能力判断单独暴露成一个**无副作用**的纯函数：页面据此决定
 * - `true`  → 渲染「在浏览器打开」按钮
 ...
 *    会让三端行为在无人察觉的情况下分叉。
 *
 * ⚠️ 也**不要**在页面里写 `const X = true` / `const X = false` 两段 `#ifdef` 来做这件事：
 *    `vue-tsc` 看到的是**条件编译前的原文**，两段里的同名 `const` 会**重复声明**、
 *    直接让 type-check 变红（第一版这么写过，已改掉）。
 */
