 ...
 * # 📣 契约变更公告栏（L1 写、前端读）
 *
 * > **为什么在这里**：L1 与前端是**两个互相够不到的会话**（GUI 单向）——
 * > 契约变了但前端不知道，只能靠需求方人工转述，实测代价是「停 48 分钟」这种量级。
 * > 这个文件是**前端每次都会读的**（类型来源就在旁边），而且 L1 本来就为两个常量改它，
 * > 所以把变更公告写在这里**零额外成本、且不依赖任何人转述**。
 *
 * ## 最近一次变更（2026-09-20，M5 交付 + CR-K/L/M）
 *
 * - 契约：**29 → 35 路径 / 63 schema**（通知 3 个端点、审核出口、举报 `POST /api/report`）。
 * - **`PostSummaryVO` / `PostDetailVO` 新增**：
 *   `liked` / `collected`（**相对于请求者**，未登录一律 false）、
 *   `imageCount`（总图数）、`imageThumbs`（**最多 3 张缩略图**，不是 9 张）、
 *   `hasDiskResource`（有网盘资源）。
